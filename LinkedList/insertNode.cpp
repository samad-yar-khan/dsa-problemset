...
node* insertNode(node*head , int a , int i){
    
    //iinsert node with value a at ith posn
    node*newNode = new node(a);

    //special case appending at head
    if(i  == 0){
        node*temp = head;
        newNode->next = temp;
        head = newNode;
...
int main(){
    ...
    printLL(head);
cout<<"\n";
    head = insertNode(head , 11 , 3);
    printLL(head);
cout<<"\n";
    head = insertNode(head , 13 , 8);
    printLL(head);
